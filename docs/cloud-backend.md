...
This means `DuckDBStore` is already a Facade — there is no missing abstraction, only a missing second implementation. **A formal `StorageBackend` Protocol/ABC is not being extracted in this document**, on the same principle this project applies elsewhere: don't add an abstraction before a second concrete need exists. A Protocol built from exactly one implementation reliably enshrines that implementation's leaks rather than describing a real contract. Concretely, three leaks a premature Protocol would either freeze or paper over:
...
**The deciding factor is direct hands-on Snowflake experience.** An integration built on genuine depth with a platform is higher quality and more representative of real capability than a generic port to an engine chosen purely for architectural neutrality. `dbt` + Snowflake is also the field's de facto standard pairing for exactly this kind of transformation-and-marts work — it directly extends the dbt project already built (`docs/dbt.md`) rather than sitting next to it as an unrelated addition.
