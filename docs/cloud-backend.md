...
**Status:** Documented, not implemented. This is a decision record for a decision **not yet made to build**, in the style of [`OPEN_QUESTIONS.md`](../OPEN_QUESTIONS.md)'s resolved sections — options, the discriminating criterion, what's blocking, what would have to be true to pick each. It is deliberately not a roadmap ("v0.3 will add Snowflake"); nothing here is scheduled.
...
**The fix that has to come first, independent of any backend decision:** a `UNIQUE` constraint on `audit_event_log.previous_hash`. This converts a fork from silent corruption into a constraint violation the appender can catch and retry against a freshly re-read tip — compare-and-swap expressed as a schema constraint. It is enforceable in DuckDB today, requires no cloud backend, and is the precondition that makes any shared backend viable later. (The empty-string genesis `previous_hash` needs a partial unique index, or an explicit one-genesis-per-chain rule, to avoid colliding two independent chains' geneses.) **This work is in scope regardless of what the rest of this document recommends, and should happen before any backend migration, not as part of one.**
